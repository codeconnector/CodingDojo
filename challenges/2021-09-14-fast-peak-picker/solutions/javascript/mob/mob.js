...
function pickPeak(input) {
    ...
    //for loop to keep track of current element
    for (let i=0; i<input.length; i++){
        //check element left and to the right
        //if current element is larger than previous && larger than the next element, return number
        //otherwise move to next element
        //
        // let leftElement = input[i-1]
        let rightElement = input[i+1]
        // let leftElementSmaller = leftElement ? leftElement < input[i] : true
        let rightElementSmaller = rightElement ? rightElement < input[i] : true 
