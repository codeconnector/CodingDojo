...
# use a STACK - last in first out - use pop
...
def play(int_list):
  ...
  # While the stack is not empty
    while stack:
        # Pop an item (List) off the stack
        sublist = stack.pop()
        # If your LIST is length 1 AND LIST[0] is 24 return True
        if len(sublist) == 1:
            if sublist[0] == 24:
                return True
            continue
        ...
        for i in range(len(sublist) - 1):
            # Do all four operations in the pair of items, collect in a list called RESULTS
            ...
            for result in results:
                # Create a list (from LIST) replacing the two input numbers with result
