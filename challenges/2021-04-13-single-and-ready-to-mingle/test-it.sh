...
if (("$tests_failed" > 0)); then
    ...
    echo -e "  - Copy the *source* (${BLU}solution.js${CLR}) to the solutions folder,"
    echo -e "    renaming it to [yourgithubusername.js]"
    echo -e "  - If you have not already created a 'yourgithubusername-wip' branch, use"
    echo -e "    ${YLW}git switch -c yourgitubusername-wip${CLR} to move your changes to"
    echo -e "    that branch"
    echo -e "  - From your WIP branch, use ${YLW}git add solutions/yourgithubusername.js${CLR}"
    echo -e "    to add your changes to the current commit"
    echo -e "  - Use ${YLW}git status${CLR} to ensure you are only committing your solution file."
    echo -e "  - Commit and push your changes: ${YLW}git commit${CLR}, then ${YLW}git push${CLR}."
    echo -e "  - Navigate to ${WHT}https://github.com/codeconnector/CodingDojo${CLR} and submit"
    echo -e "    your pull request"
    echo -e "\n"
fi
