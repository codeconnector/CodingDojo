...
function currentLocation(start, square) {
  ...
  if (square < 1) {
    throw new Error("Square is less than one");
  }
  ...
  let current_path_steps = 1;
  let steps_taken = 0;
  let total_steps_taken = 1;
  let should_we_increase_steps = false;

  while (total_steps_taken < square) {
    start[0] += y;
    start[1] += x;

    steps_taken++;

    if (steps_taken >= current_path_steps) {
      steps_taken = 0;

      [x, y] = [y, -x];

      if (should_we_increase_steps) {
        current_path_steps += 1;
        should_we_increase_steps = false;
      } else {
        should_we_increase_steps = true;
      }
    }
